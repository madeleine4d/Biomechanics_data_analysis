...
# create main dataframe that will hold data for all participant. This will be replaced by an existing file is 'load' command by user
DATA = pd.DataFrame(
...
def Fileloader(path, YNformated=False):
    match YNformated:
        case True:
            if path[-5:] == ".xlsx":
                ...
                raise (
                    ImportError(
                        Fore.RED
                        + "file type not supported. Please double check you typed the path correctly."
                        + Style.RESET_ALL
                    )
                )
        case False:
...
def FindGreatestValue(dataframe, columnName):
    return max([abs(float(num)) for num in dataframe[columnName].tolist()])
...
def FindAbsMean(dataframe, columnName):
    print(dataframe)
    absolutes = []
    for num in dataframe[columnName].tolist():
        print(num)
        if not math.isnan(num):
            absolutes.append(abs(num))
...
def TTest(array1: list, array2: list, arrayname1: str, arrayname2: str):
    ...
    tTestString = (
        "statistic: "
        + str(getattr(tTest, "statistic"))
        + "\np-value: "
        + str(getattr(tTest, "pvalue"))
        + "\ndegrees of freedom: "
        + str(getattr(tTest, "df"))
    )
    with open(
        "t-test_"
        + arrayname1.replace(" ", "_")
        + "_vs_"
        + arrayname2.replace(" ", "_")
        + ".txt",
        "w",
    ) as text_file:
...
def PrintHelp():
    print(
        """What would you like to do? 
          ...
          'load' (if you have a master data file you have been using, this comands allows you to import it)
          ...
          'show' (print the entire loaded data. add -S to only show a several line summery)
          'export'
    ...
    try:
        match currentCommand:
            case "load":
                DATA = Fileloader(
                    input(
                        "Ender the path to the file holding the data you are processing:\n"
                    ),
                    True,
                )
                print("file loaded")
            ...
            case "MVC" | "MVC -R" | "DLS" | "DLS -R" | "SLS" | "SLS -R":
                ...
                participant = input(
                    "Enter the full name of the participant for whom you are adding a "
                    + currentCommand.split(" -")[0]
                )
                match currentCommand:
                    case "MVC":
                        AddEntry(
                            participant,
                            "MVC max",
                            FindGreatestValue(tempData, "mVs"),
                            False,
                        )
                    case "MVC -R":
                        AddEntry(
                    ...
                    case "DLS":
                        AddEntry(
                            participant, "DLS mean", FindAbsMean(tempData, "mVs"), False
                        )
                    case "DLS -R":
                        AddEntry(
                            participant, "DLS mean", FindAbsMean(tempData, "mVs"), True
                        )
                    case "SLS":
                        AddEntry(
                            participant, "SLS mean", FindAbsMean(tempData, "mVs"), False
                        )
                    case "SLS -R":
                        AddEntry(
                            participant, "SLS mean", FindAbsMean(tempData, "mVs"), True
                        )

            case "MVC -M" | "DLS -M" | "SLS -M":
                subcommand = input(
                    "Enter a comma separated list of paths to the "
                    + currentCommand.split(" -")[0]
                ...
                for i in range(len(subpaths)):
                    tempData = Fileloader(subpaths[i])
                    tempData = DataFormater(tempData)
                    match currentCommand:
                        case "MVC -M":
                            AddEntry(
            ...
            case "t-test":
                subcommand = input(
                    "please enter the names of the columns you would like to compare separated with commas:\n"
                ).split(", ")
                TTest(
                    DATA[subcommand[0]].dropna().tolist(),
                    DATA[subcommand[1]].dropna().tolist(),
                    subcommand[0],
                    subcommand[1],
                )
            case "export":
                Export(
                    DATA,
                    input("Please enter the path and file name you would like: \n"),
                )
            case "help":
                PrintHelp()
            case "show":
